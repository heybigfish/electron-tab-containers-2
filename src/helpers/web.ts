 ...
 * 容器 ID 集合字段
 */
export const kContainerIdsKey =
  'kDesktopContainerIdsKey'
...
/**
 * 获取 preload-dialog 路径
 */
export function getDialogPreloadPath(): string {
  return path.join(__dirname, 'preload-dialog.js')
}
/**
 * 拦截打开 window 事件
 * @param webContents
...
export function handleOpenWindow(
  ...
  webContents.setWindowOpenHandler(
    ({ url }) => {
      GDTabPageContainer.shared.switchTab(
        url
      )
      return {
        action // deny,allow
 ...
 */
export function startDevToolsIfNeed(
  webContents: WebContents
) {
  if (!app.isPackaged) {
    let clicks = 0
